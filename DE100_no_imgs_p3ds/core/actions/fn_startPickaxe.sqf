...

{
	...
	nil;
} count ("!(getText(_x >> 'item') isEqualTo '') && (getNumber(_x >> 'mineral') isEqualTo 1)" configClasses (missionConfigFile >> "resourceFields"));
...
if (_cAdd == 0) exitWith {[localize "STR_NOTF_InvFull",true,"fast"] call life_fnc_notification_system;life_action_inUse = false;};
private _header = if (_altStat) then {
	format["Baue %1 ab (Automatisch)",localize _itemName];
} else {
	format["Baue %1 ab",localize _itemName];
};
...
[_pData select 0,_header,{!isNull objectParent player || !life_action_inUse},[_altStat,_item,_cAdd,0],life_fnc_endPickaxe,life_fnc_endPickaxe] spawn life_fnc_progressBar;