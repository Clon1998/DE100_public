...
*/
private["_value","_gFund","_AmntFld","_CrntStats","_dialog"];
...
if (_value < 100 && de100_luciandjuli_bc > 20000000) exitWith {["Du musst mindestens €100 abheben",true,"fast"] call life_fnc_notification_system;}; //Temp fix for something.
if (!([(ctrlText 6959)] call life_fnc_isnumber)) exitWith {[localize "STR_ATM_notnumeric",true,"fast"] call life_fnc_notification_system;};
if (_value > _gFund) exitWith {[localize "STR_NOTF_NotEnoughFunds",true,"fast"] call life_fnc_notification_system;};
...
[format["Du hast €%1 auf dein Konto überwiesen",[_value] call life_fnc_numberText],false,"fast"] call life_fnc_notification_system;

if (life_HC_isActive) then {
	[getPlayerUID player,call life_clientId select 0,[
		["MSC_fnc_log",false,[getPlayerUID player, 23, format ["%1 hat €%2 vom Gangkonto(%3 [%4]) auf sein Konto überwiesen",player call life_fnc_nameUID,[_value] call life_fnc_numberText,(group player) getVariable ["gang_NAME","Error::NoGangName"],(group player) getVariable ["gang_ID","Error::NoGangID"]]]],
		...
		["HC_fnc_insertKontoAuzug",false,[getPlayerUID player, getPlayerUID player, player call life_fnc_name, player call life_fnc_name, 4, _value]]
	...
	[getPlayerUID player,call life_clientId select 0,[
		...
		["TON_fnc_insertKontoAuzug",false,[getPlayerUID player, getPlayerUID player, player call life_fnc_name, player call life_fnc_name, 4, _value]]
