...
(_display displayCtrl 4002) ctrlSetText format["%1/%2",_display getVariable ["used_clothing_space",0],_display getVariable ["available_space",0]];
(_display displayCtrl 4004) ctrlSetText format[""];

private _var = if (_house isEqualTo lockers_obj) then {
	format["%1_%2_locker_clothing_space",getPlayerUID player,playerSide];
} else {
	"clothing_space";
};
