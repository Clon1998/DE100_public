...
lnbClear _houseLB;
lnbClear _playerLB;
if (_key in ["rifle","sniper","pistole"]) then {	
	_houseLB lnbSetColumnsPos [-0.01,0.033 * safezoneW,0.066 * safezoneW];
	_playerLB lnbSetColumnsPos [-0.01,0.033 * safezoneW,0.066 * safezoneW];
} else {
...
(_display displayCtrl 4002) ctrlSetText format["%1/%2",_display getVariable ["used_weapon_space",0],_display getVariable ["available_space",0]];
(_display displayCtrl 4004) ctrlSetText format[""];

private _var = if (_house isEqualTo lockers_obj) then {
	format["%1_%2_locker_weapon_space",getPlayerUID player,playerSide];
} else {
	"weapon_space";
};
...
_allowedTypes = switch (_key) do { 
	case "rifle" : {["AssaultRifle","MachineGun","Rifle","SubmachineGun"];}; 
	case "sniper" : {["SniperRifle"];}; 
	case "pistole" : {["Handgun"];}; 
	case "scope" : {["AccessorySights"];}; 
	case "muzzle" : {["AccessoryMuzzle"];}; 
	case "siderail" : {["AccessoryPointer"];}; 
	case "bipod" : {["AccessoryBipod"];}; 
	case "magazine" : {["Bullet","Shell"];}; 
	case "grenate" : {["SmokeShell","Grenade","Flare","Artillery"];}; 
	case "binocular" : {["Binocular","LaserDesignator","NVGoggles"];}; 
};
