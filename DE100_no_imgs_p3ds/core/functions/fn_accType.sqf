	...
	if(_x != "") then
	{
		...
		if(count _legacyItems > 0) then
		{
			...
			if((toLower _item) in _legacyItems) exitWith {
				_ret = call {if (_weapon isEqualTo (primaryWeapon player)) exitWith {1;};if (_weapon isEqualTo (secondaryWeapon player)) exitWith {2;};if (_weapon isEqualTo (handgunWeapon player)) exitWith {3;};0;};
			};
		};
