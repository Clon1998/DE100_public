/*
	...
	16: acc_bipot
*/
private["_acc_b","_mass","_entity","_cfg","_ret","_type","_acc_p","_acc_o","_slotclasses","_acc_m","_scope","_displayName","_picture","_config","_itemInfo","_muzzles","_magazines","_desc","_base"];
...
switch (_cfg) do
{
	...
	case "CfgWeapons":
	{
		...
		if(isClass (_config >> "WeaponSlotsInfo")) then
		{
			...
			_acc_b = getArray(_config >> "WeaponSlotsInfo" >> "UnderBarrelSlot" >> "compatibleItems");
		...
		_tmp = getNumber(_config >> "Type");
		if (_tmp in [1,2,4,4096]) then {
...
_ret = [_entity,_displayName,_picture,_scope,_type,_itemInfo,_cfg,_magazines,_muzzles,_desc,_acc_p,_acc_o,_acc_m,_base,_slotclasses,_mass,_acc_b];
_ret;