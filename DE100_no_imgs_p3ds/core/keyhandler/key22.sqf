...
if (time - life_key_delay < 1) exitWith {_stop};
life_key_delay = time;
...
if(!_altk && !_ctrl) then {
	...
	if(_veh isKindOf "House_F" && playerSide == civilian) then {
		if(_veh in life_vehicles && (([_veh,player] call life_fnc_bBoxDistance) < 8 || _veh isKindOf "House_F")) then {
			if (_veh getVariable ["isStorehouse",false] && !("HOUSE" in _playerPermissions)) exitwith {["Du hast keine Berechtigung, um auf das Lagerhaus zuzugreifen.",true,"fast"] call life_fnc_notification_system;};
			if (_veh getVariable ["isStorehouse",false] && !(((_veh getVariable ["house_owner",["",""]]) select 0) isEqualTo (_gang getVariable ["gang_ID",0]))) exitwith {["Nur Gangmitglieder können auf das Lagerhaus zugreifen.",true,"fast"] call life_fnc_notification_system;};
		...
		_locked = locked _veh;
		if(_veh in life_vehicles && (([_veh,player] call life_fnc_bBoxDistance) < 8 || !isNull objectParent player)) then {
			if(_locked == 2) then {
				if(local _veh) then {
					_veh lock 0;
