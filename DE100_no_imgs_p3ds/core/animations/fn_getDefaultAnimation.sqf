...
private _stance = call {
    if ((_animState select [4, 4]) isEqualTo "ppne") exitWith {"pne";};
    if ((_animState select [4, 4]) isEqualTo "pknl") exitWith {"knl";};
    if ((_animState select [4, 4]) isEqualTo "perc") exitWith {"erc";};
